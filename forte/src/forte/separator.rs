...
impl SeparatorBuilder {
    ...
    pub fn bind(&mut self) -> SeparatorBinder {
        SeparatorBinder { builder: self }
    }
    pub fn connect(&mut self) -> SeparatorSignals {
        SeparatorSignals { builder: self }
    }
}
impl crate::prelude::Builder for SeparatorBuilder {
...
impl std::ops::Deref for SeparatorBuilder {
    type Target = Separator;
    fn deref(&self) -> &Self::Target {
        &self.obj
    }
}
pub struct SeparatorBinder<'builder> {
    builder: &'builder mut SeparatorBuilder,
}
impl<'builder> SeparatorBinder<'builder> {
    pub fn margin_bottom(&mut self, value: &(impl Prop<i32> + ?Sized)) -> &mut SeparatorBuilder {
    ...
    pub fn name(&mut self, value: &(impl Prop<str> + ?Sized)) -> &mut SeparatorBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("name", val));
        value.connect_update(move |value| obj.set_property("name", value));
        self.builder
    }
    pub fn hexpand(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut SeparatorBuilder {
        let obj = self.builder.obj.clone();
...
pub struct SeparatorSignals<'builder> {
    builder: &'builder mut SeparatorBuilder,
}
impl<'builder> SeparatorSignals<'builder> {}
impl ForteExt for Separator {
    type Builder = SeparatorBuilder;
}
#[macro_export]
macro_rules ! Separator { { $ ($ rest : tt) * } => { forte ! { (gtkrs :: Separator :: forte ()) $ ($ rest) * } } }
