...
impl GestureClickBuilder {
    ...
    pub fn bind(&mut self) -> GestureClickBinder {
        GestureClickBinder { builder: self }
    }
    pub fn connect(&mut self) -> GestureClickSignals {
        GestureClickSignals { builder: self }
    }
}
impl crate::prelude::Builder for GestureClickBuilder {
    type Target = GestureClick;
...
impl std::ops::Deref for GestureClickBuilder {
    type Target = GestureClick;
    fn deref(&self) -> &Self::Target {
        &self.obj
    }
}
pub struct GestureClickBinder<'builder> {
    builder: &'builder mut GestureClickBuilder,
}
impl<'builder> GestureClickBinder<'builder> {
    pub fn propagation_limit(
        &mut self,
        value: &(impl Prop<PropagationLimit> + ?Sized),
    ) -> &mut GestureClickBuilder {
...
pub struct GestureClickSignals<'builder> {
    builder: &'builder mut GestureClickBuilder,
}
impl<'builder> GestureClickSignals<'builder> {}
impl ForteExt for GestureClick {
    type Builder = GestureClickBuilder;
}
#[macro_export]
macro_rules ! GestureClick { { $ ($ rest : tt) * } => { forte ! { (gtkrs :: GestureClick :: forte ()) $ ($ rest) * } } }
