...
use gtkrs::{prelude::*, ColorChooserDialog, *};
use gtkrs::{
...
impl ColorChooserDialogBuilder {
    ...
    pub fn bind(&mut self) -> ColorChooserDialogBinder {
        ColorChooserDialogBinder { builder: self }
    }
    pub fn connect(&mut self) -> ColorChooserDialogSignals {
        ColorChooserDialogSignals { builder: self }
    }
}
impl crate::prelude::Builder for ColorChooserDialogBuilder {
    type Target = ColorChooserDialog;
...
impl std::ops::Deref for ColorChooserDialogBuilder {
    type Target = ColorChooserDialog;
    fn deref(&self) -> &Self::Target {
        &self.obj
    }
}
pub struct ColorChooserDialogBinder<'builder> {
    builder: &'builder mut ColorChooserDialogBuilder,
}
impl<'builder> ColorChooserDialogBinder<'builder> {
    pub fn hexpand(
        &mut self,
        value: &(impl Prop<bool> + ?Sized),
    ) -> &mut ColorChooserDialogBuilder {
    ...
    pub fn opacity(&mut self, value: &(impl Prop<f64> + ?Sized)) -> &mut ColorChooserDialogBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn name(&mut self, value: &(impl Prop<str> + ?Sized)) -> &mut ColorChooserDialogBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn modal(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut ColorChooserDialogBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn title(&mut self, value: &(impl Prop<str> + ?Sized)) -> &mut ColorChooserDialogBuilder {
        let obj = self.builder.obj.clone();
...
pub struct ColorChooserDialogSignals<'builder> {
    builder: &'builder mut ColorChooserDialogBuilder,
}
impl<'builder> ColorChooserDialogSignals<'builder> {}
impl ForteExt for ColorChooserDialog {
    type Builder = ColorChooserDialogBuilder;
}
#[macro_export]
macro_rules ! ColorChooserDialog { { $ ($ rest : tt) * } => { forte ! { (gtkrs :: ColorChooserDialog :: forte ()) $ ($ rest) * } } }
