...
impl InfoBarBuilder {
    ...
    pub fn revealed(&mut self, value: bool) -> &mut Self {
        self.obj.set_property("revealed", value);
        self
    }
    ...
    pub fn bind(&mut self) -> InfoBarBinder {
        InfoBarBinder { builder: self }
    }
    pub fn connect(&mut self) -> InfoBarSignals {
        InfoBarSignals { builder: self }
    }
}
impl crate::prelude::Builder for InfoBarBuilder {
    type Target = InfoBar;
...
impl std::ops::Deref for InfoBarBuilder {
    type Target = InfoBar;
    fn deref(&self) -> &Self::Target {
        &self.obj
    }
}
pub struct InfoBarBinder<'builder> {
    builder: &'builder mut InfoBarBuilder,
}
impl<'builder> InfoBarBinder<'builder> {
    pub fn receives_default(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut InfoBarBuilder {
    ...
    pub fn css_name(&mut self, value: &(impl Prop<str> + ?Sized)) -> &mut InfoBarBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("css_name", val));
        value.connect_update(move |value| obj.set_property("css_name", value));
        self.builder
    }
    pub fn hexpand_set(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut InfoBarBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn opacity(&mut self, value: &(impl Prop<f64> + ?Sized)) -> &mut InfoBarBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("opacity", val));
        value.connect_update(move |value| obj.set_property("opacity", value));
        self.builder
    }
    pub fn vexpand(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut InfoBarBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn margin_end(&mut self, value: &(impl Prop<i32> + ?Sized)) -> &mut InfoBarBuilder {
        let obj = self.builder.obj.clone();
...
pub struct InfoBarSignals<'builder> {
    builder: &'builder mut InfoBarBuilder,
}
impl<'builder> InfoBarSignals<'builder> {}
impl ForteExt for InfoBar {
    type Builder = InfoBarBuilder;
}
#[macro_export]
macro_rules ! InfoBar { { $ ($ rest : tt) * } => { forte ! { (gtkrs :: InfoBar :: forte ()) $ ($ rest) * } } }
