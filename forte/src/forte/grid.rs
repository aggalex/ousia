...
impl GridBuilder {
    ...
    pub fn bind(&mut self) -> GridBinder {
        GridBinder { builder: self }
    }
    pub fn connect(&mut self) -> GridSignals {
        GridSignals { builder: self }
    }
}
impl crate::prelude::Builder for GridBuilder {
    type Target = Grid;
...
impl std::ops::Deref for GridBuilder {
    type Target = Grid;
    fn deref(&self) -> &Self::Target {
        &self.obj
    }
}
pub struct GridBinder<'builder> {
    builder: &'builder mut GridBuilder,
}
impl<'builder> GridBinder<'builder> {
    pub fn can_target(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut GridBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("can_target", val));
        value.connect_update(move |value| obj.set_property("can_target", value));
        self.builder
    }
    pub fn receives_default(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut GridBuilder {
        let obj = self.builder.obj.clone();
...
pub struct GridSignals<'builder> {
    builder: &'builder mut GridBuilder,
}
impl<'builder> GridSignals<'builder> {
    pub fn row_spacing_notify(&mut self, f: impl Fn(&Grid) + 'static) -> &mut GridBuilder {
        let obj = self.builder.obj.clone();
        obj.connect_row_spacing_notify(f);
        &mut self.builder
    }
    pub fn column_spacing_notify(&mut self, f: impl Fn(&Grid) + 'static) -> &mut GridBuilder {
        let obj = self.builder.obj.clone();
        obj.connect_column_spacing_notify(f);
        &mut self.builder
    }
    pub fn row_homogeneous_notify(&mut self, f: impl Fn(&Grid) + 'static) -> &mut GridBuilder {
        let obj = self.builder.obj.clone();
        obj.connect_row_homogeneous_notify(f);
        &mut self.builder
    }
    pub fn baseline_row_notify(&mut self, f: impl Fn(&Grid) + 'static) -> &mut GridBuilder {
        let obj = self.builder.obj.clone();
        obj.connect_baseline_row_notify(f);
        &mut self.builder
    }
    pub fn column_homogeneous_notify(&mut self, f: impl Fn(&Grid) + 'static) -> &mut GridBuilder {
        let obj = self.builder.obj.clone();
        obj.connect_column_homogeneous_notify(f);
        &mut self.builder
    }
}
impl ForteExt for Grid {
    type Builder = GridBuilder;
}
#[macro_export]
macro_rules ! Grid { { $ ($ rest : tt) * } => { forte ! { (gtkrs :: Grid :: forte ()) $ ($ rest) * } } }
