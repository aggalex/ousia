...
impl FrameBuilder {
    ...
    pub fn label_xalign(&mut self, value: f32) -> &mut Self {
        self.obj.set_property("label_xalign", value);
        self
    }
    ...
    pub fn bind(&mut self) -> FrameBinder {
        FrameBinder { builder: self }
    }
    pub fn connect(&mut self) -> FrameSignals {
        FrameSignals { builder: self }
    }
}
impl crate::prelude::Builder for FrameBuilder {
    type Target = Frame;
...
impl std::ops::Deref for FrameBuilder {
    type Target = Frame;
    fn deref(&self) -> &Self::Target {
        &self.obj
    }
}
pub struct FrameBinder<'builder> {
    builder: &'builder mut FrameBuilder,
}
impl<'builder> FrameBinder<'builder> {
    pub fn receives_default(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut FrameBuilder {
    ...
    pub fn tooltip_text(&mut self, value: &(impl Prop<str> + ?Sized)) -> &mut FrameBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("tooltip_text", val));
        value.connect_update(move |value| obj.set_property("tooltip_text", value));
        self.builder
    }
    pub fn label_xalign(&mut self, value: &(impl Prop<f32> + ?Sized)) -> &mut FrameBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("label_xalign", val));
        value.connect_update(move |value| obj.set_property("label_xalign", value));
        self.builder
    }
...
pub struct FrameSignals<'builder> {
    builder: &'builder mut FrameBuilder,
}
impl<'builder> FrameSignals<'builder> {
    pub fn label_xalign_notify(&mut self, f: impl Fn(&Frame) + 'static) -> &mut FrameBuilder {
        let obj = self.builder.obj.clone();
        obj.connect_label_xalign_notify(f);
        &mut self.builder
    }
    pub fn child_notify(&mut self, f: impl Fn(&Frame) + 'static) -> &mut FrameBuilder {
        let obj = self.builder.obj.clone();
        obj.connect_child_notify(f);
        &mut self.builder
    }
    pub fn label_notify(&mut self, f: impl Fn(&Frame) + 'static) -> &mut FrameBuilder {
        let obj = self.builder.obj.clone();
        obj.connect_label_notify(f);
        &mut self.builder
    }
    pub fn label_widget_notify(&mut self, f: impl Fn(&Frame) + 'static) -> &mut FrameBuilder {
        let obj = self.builder.obj.clone();
        obj.connect_label_widget_notify(f);
        &mut self.builder
    }
}
impl ForteExt for Frame {
    type Builder = FrameBuilder;
}
#[macro_export]
macro_rules ! Frame { { $ ($ rest : tt) * } => { forte ! { (gtkrs :: Frame :: forte ()) $ ($ rest) * } } }
