...
impl OverlayBuilder {
    ...
    pub fn bind(&mut self) -> OverlayBinder {
        OverlayBinder { builder: self }
    }
    pub fn connect(&mut self) -> OverlaySignals {
        OverlaySignals { builder: self }
    }
}
impl crate::prelude::Builder for OverlayBuilder {
    type Target = Overlay;
...
impl std::ops::Deref for OverlayBuilder {
    type Target = Overlay;
    fn deref(&self) -> &Self::Target {
        &self.obj
    }
}
pub struct OverlayBinder<'builder> {
    builder: &'builder mut OverlayBuilder,
}
impl<'builder> OverlayBinder<'builder> {
    ...
    pub fn cursor(&mut self, value: &(impl Prop<gdk::Cursor> + ?Sized)) -> &mut OverlayBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("cursor", val));
        value.connect_update(move |value| obj.set_property("cursor", value));
        self.builder
    }
    pub fn margin_bottom(&mut self, value: &(impl Prop<i32> + ?Sized)) -> &mut OverlayBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("margin_bottom", val));
        value.connect_update(move |value| obj.set_property("margin_bottom", value));
        self.builder
    }
    pub fn focus_on_click(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut OverlayBuilder {
        let obj = self.builder.obj.clone();
...
pub struct OverlaySignals<'builder> {
    builder: &'builder mut OverlayBuilder,
}
impl<'builder> OverlaySignals<'builder> {}
impl ForteExt for Overlay {
    type Builder = OverlayBuilder;
}
#[macro_export]
macro_rules ! Overlay { { $ ($ rest : tt) * } => { forte ! { (gtkrs :: Overlay :: forte ()) $ ($ rest) * } } }
