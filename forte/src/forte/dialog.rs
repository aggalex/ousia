...
impl DialogBuilder {
    ...
    pub fn bind(&mut self) -> DialogBinder {
        DialogBinder { builder: self }
    }
    pub fn connect(&mut self) -> DialogSignals {
        DialogSignals { builder: self }
    }
}
impl crate::prelude::Builder for DialogBuilder {
    type Target = Dialog;
...
impl std::ops::Deref for DialogBuilder {
    type Target = Dialog;
    fn deref(&self) -> &Self::Target {
        &self.obj
    }
}
pub struct DialogBinder<'builder> {
    builder: &'builder mut DialogBuilder,
}
impl<'builder> DialogBinder<'builder> {
    pub fn use_header_bar(&mut self, value: &(impl Prop<i32> + ?Sized)) -> &mut DialogBuilder {
    ...
    pub fn mnemonics_visible(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut DialogBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn margin_bottom(&mut self, value: &(impl Prop<i32> + ?Sized)) -> &mut DialogBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn margin_start(&mut self, value: &(impl Prop<i32> + ?Sized)) -> &mut DialogBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn valign(&mut self, value: &(impl Prop<Align> + ?Sized)) -> &mut DialogBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn width_request(&mut self, value: &(impl Prop<i32> + ?Sized)) -> &mut DialogBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn tooltip_markup(&mut self, value: &(impl Prop<str> + ?Sized)) -> &mut DialogBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("tooltip_markup", val));
        value.connect_update(move |value| obj.set_property("tooltip_markup", value));
        self.builder
    }
    pub fn modal(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut DialogBuilder {
        let obj = self.builder.obj.clone();
...
pub struct DialogSignals<'builder> {
    builder: &'builder mut DialogBuilder,
}
impl<'builder> DialogSignals<'builder> {
    pub fn close(&mut self, f: impl Fn(&Dialog) + 'static) -> &mut DialogBuilder {
        let obj = self.builder.obj.clone();
        obj.connect_close(f);
        &mut self.builder
    }
    pub fn response(&mut self, f: impl Fn(&Dialog, ResponseType) + 'static) -> &mut DialogBuilder {
        let obj = self.builder.obj.clone();
        obj.connect_response(f);
        &mut self.builder
    }
}
impl ForteExt for Dialog {
    type Builder = DialogBuilder;
}
#[macro_export]
macro_rules ! Dialog { { $ ($ rest : tt) * } => { forte ! { (gtkrs :: Dialog :: forte ()) $ ($ rest) * } } }
