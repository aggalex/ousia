...
impl ScrollbarBuilder {
    ...
    pub fn bind(&mut self) -> ScrollbarBinder {
        ScrollbarBinder { builder: self }
    }
    pub fn connect(&mut self) -> ScrollbarSignals {
        ScrollbarSignals { builder: self }
    }
}
impl crate::prelude::Builder for ScrollbarBuilder {
...
impl std::ops::Deref for ScrollbarBuilder {
    type Target = Scrollbar;
    fn deref(&self) -> &Self::Target {
        &self.obj
    }
}
pub struct ScrollbarBinder<'builder> {
    builder: &'builder mut ScrollbarBuilder,
}
impl<'builder> ScrollbarBinder<'builder> {
    pub fn width_request(&mut self, value: &(impl Prop<i32> + ?Sized)) -> &mut ScrollbarBuilder {
    ...
    pub fn can_target(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut ScrollbarBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn valign(&mut self, value: &(impl Prop<Align> + ?Sized)) -> &mut ScrollbarBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn tooltip_markup(&mut self, value: &(impl Prop<str> + ?Sized)) -> &mut ScrollbarBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("tooltip_markup", val));
        value.connect_update(move |value| obj.set_property("tooltip_markup", value));
        self.builder
    }
    pub fn vexpand(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut ScrollbarBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn focusable(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut ScrollbarBuilder {
        let obj = self.builder.obj.clone();
...
pub struct ScrollbarSignals<'builder> {
    builder: &'builder mut ScrollbarBuilder,
}
impl<'builder> ScrollbarSignals<'builder> {}
impl ForteExt for Scrollbar {
    type Builder = ScrollbarBuilder;
}
#[macro_export]
macro_rules ! Scrollbar { { $ ($ rest : tt) * } => { forte ! { (gtkrs :: Scrollbar :: forte ()) $ ($ rest) * } } }
