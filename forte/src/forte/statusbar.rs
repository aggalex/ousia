...
impl StatusbarBuilder {
    ...
    pub fn bind(&mut self) -> StatusbarBinder {
        StatusbarBinder { builder: self }
    }
    pub fn connect(&mut self) -> StatusbarSignals {
        StatusbarSignals { builder: self }
    }
}
impl crate::prelude::Builder for StatusbarBuilder {
...
impl<'builder> StatusbarBinder<'builder> {
    ...
    pub fn css_name(&mut self, value: &(impl Prop<str> + ?Sized)) -> &mut StatusbarBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn valign(&mut self, value: &(impl Prop<Align> + ?Sized)) -> &mut StatusbarBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("valign", val));
        value.connect_update(move |value| obj.set_property("valign", value));
        self.builder
    }
    pub fn can_focus(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut StatusbarBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("can_focus", val));
        value.connect_update(move |value| obj.set_property("can_focus", value));
        self.builder
    }
    pub fn tooltip_markup(&mut self, value: &(impl Prop<str> + ?Sized)) -> &mut StatusbarBuilder {
        let obj = self.builder.obj.clone();
...
pub struct StatusbarSignals<'builder> {
    builder: &'builder mut StatusbarBuilder,
}
impl<'builder> StatusbarSignals<'builder> {}
impl ForteExt for Statusbar {
    type Builder = StatusbarBuilder;
}
#[macro_export]
macro_rules ! Statusbar { { $ ($ rest : tt) * } => { forte ! { (gtkrs :: Statusbar :: forte ()) $ ($ rest) * } } }
