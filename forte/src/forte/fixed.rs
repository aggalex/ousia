...
impl FixedBuilder {
    ...
    pub fn bind(&mut self) -> FixedBinder {
        FixedBinder { builder: self }
    }
    pub fn connect(&mut self) -> FixedSignals {
        FixedSignals { builder: self }
    }
}
impl crate::prelude::Builder for FixedBuilder {
    type Target = Fixed;
...
impl std::ops::Deref for FixedBuilder {
    type Target = Fixed;
    fn deref(&self) -> &Self::Target {
        &self.obj
    }
}
pub struct FixedBinder<'builder> {
    builder: &'builder mut FixedBuilder,
}
impl<'builder> FixedBinder<'builder> {
    pub fn hexpand(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut FixedBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("hexpand", val));
        value.connect_update(move |value| obj.set_property("hexpand", value));
        self.builder
    }
    pub fn margin_end(&mut self, value: &(impl Prop<i32> + ?Sized)) -> &mut FixedBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("margin_end", val));
        value.connect_update(move |value| obj.set_property("margin_end", value));
        self.builder
    }
    pub fn receives_default(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut FixedBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn focusable(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut FixedBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("focusable", val));
        value.connect_update(move |value| obj.set_property("focusable", value));
        self.builder
    }
    pub fn tooltip_markup(&mut self, value: &(impl Prop<str> + ?Sized)) -> &mut FixedBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn vexpand(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut FixedBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("vexpand", val));
        value.connect_update(move |value| obj.set_property("vexpand", value));
        self.builder
    }
    pub fn can_target(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut FixedBuilder {
        let obj = self.builder.obj.clone();
...
pub struct FixedSignals<'builder> {
    builder: &'builder mut FixedBuilder,
}
impl<'builder> FixedSignals<'builder> {}
impl ForteExt for Fixed {
    type Builder = FixedBuilder;
}
#[macro_export]
macro_rules ! Fixed { { $ ($ rest : tt) * } => { forte ! { (gtkrs :: Fixed :: forte ()) $ ($ rest) * } } }
