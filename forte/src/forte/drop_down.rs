...
impl DropDownBuilder {
    ...
    pub fn show_arrow(&mut self, value: bool) -> &mut Self {
        self.obj.set_property("show_arrow", value);
        self
    }
    ...
    pub fn list_factory(&mut self, value: &impl IsA<ListItemFactory>) -> &mut Self {
        self.obj.set_property("list_factory", value);
        self
    }
    ...
    pub fn bind(&mut self) -> DropDownBinder {
        DropDownBinder { builder: self }
    }
    pub fn connect(&mut self) -> DropDownSignals {
        DropDownSignals { builder: self }
    }
}
impl crate::prelude::Builder for DropDownBuilder {
...
impl std::ops::Deref for DropDownBuilder {
    type Target = DropDown;
    fn deref(&self) -> &Self::Target {
        &self.obj
    }
}
pub struct DropDownBinder<'builder> {
    builder: &'builder mut DropDownBuilder,
}
impl<'builder> DropDownBinder<'builder> {
    pub fn can_focus(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut DropDownBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("can_focus", val));
        value.connect_update(move |value| obj.set_property("can_focus", value));
        self.builder
    }
    pub fn margin_end(&mut self, value: &(impl Prop<i32> + ?Sized)) -> &mut DropDownBuilder {
        let obj = self.builder.obj.clone();
    ...
    #[cfg(any(feature = "v4_6", feature = "dox"))]
    #[cfg_attr(feature = "dox", doc(cfg(feature = "v4_6")))]
    pub fn show_arrow(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut DropDownBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("show_arrow", val));
        value.connect_update(move |value| obj.set_property("show_arrow", value));
        self.builder
    }
    pub fn opacity(&mut self, value: &(impl Prop<f64> + ?Sized)) -> &mut DropDownBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("opacity", val));
        value.connect_update(move |value| obj.set_property("opacity", value));
        self.builder
    }
    pub fn tooltip_markup(&mut self, value: &(impl Prop<str> + ?Sized)) -> &mut DropDownBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("tooltip_markup", val));
        value.connect_update(move |value| obj.set_property("tooltip_markup", value));
        self.builder
    }
    pub fn list_factory<T: IsA<ListItemFactory>>(
        &mut self,
        value: &(impl Prop<T> + ?Sized),
    ) -> &mut DropDownBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("list_factory", val));
        value.connect_update(move |value| obj.set_property("list_factory", value));
        self.builder
    }
    pub fn enable_search(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut DropDownBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("enable_search", val));
        value.connect_update(move |value| obj.set_property("enable_search", value));
        self.builder
    }
    pub fn halign(&mut self, value: &(impl Prop<Align> + ?Sized)) -> &mut DropDownBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("halign", val));
        value.connect_update(move |value| obj.set_property("halign", value));
        self.builder
    }
    pub fn hexpand(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut DropDownBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("hexpand", val));
        value.connect_update(move |value| obj.set_property("hexpand", value));
        self.builder
    }
    pub fn sensitive(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut DropDownBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("sensitive", val));
        value.connect_update(move |value| obj.set_property("sensitive", value));
        self.builder
    }
    pub fn focusable(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut DropDownBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("focusable", val));
        value.connect_update(move |value| obj.set_property("focusable", value));
        self.builder
    }
    pub fn selected(&mut self, value: &(impl Prop<u32> + ?Sized)) -> &mut DropDownBuilder {
        let obj = self.builder.obj.clone();
...
pub struct DropDownSignals<'builder> {
    builder: &'builder mut DropDownBuilder,
}
impl<'builder> DropDownSignals<'builder> {}
impl ForteExt for DropDown {
    type Builder = DropDownBuilder;
}
#[macro_export]
macro_rules ! DropDown { { $ ($ rest : tt) * } => { forte ! { (gtkrs :: DropDown :: forte ()) $ ($ rest) * } } }
