...
use gtkrs::{prelude::*, FlowBoxChild, *};
use gtkrs::{
...
impl FlowBoxChildBuilder {
    ...
    pub fn bind(&mut self) -> FlowBoxChildBinder {
        FlowBoxChildBinder { builder: self }
    }
    pub fn connect(&mut self) -> FlowBoxChildSignals {
        FlowBoxChildSignals { builder: self }
    }
}
impl crate::prelude::Builder for FlowBoxChildBuilder {
    type Target = FlowBoxChild;
...
impl std::ops::Deref for FlowBoxChildBuilder {
    type Target = FlowBoxChild;
    fn deref(&self) -> &Self::Target {
        &self.obj
    }
}
pub struct FlowBoxChildBinder<'builder> {
    builder: &'builder mut FlowBoxChildBuilder,
}
impl<'builder> FlowBoxChildBinder<'builder> {
    pub fn vexpand(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut FlowBoxChildBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("vexpand", val));
        value.connect_update(move |value| obj.set_property("vexpand", value));
        self.builder
    }
    pub fn margin_bottom(&mut self, value: &(impl Prop<i32> + ?Sized)) -> &mut FlowBoxChildBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("margin_bottom", val));
        value.connect_update(move |value| obj.set_property("margin_bottom", value));
        self.builder
    }
    pub fn margin_start(&mut self, value: &(impl Prop<i32> + ?Sized)) -> &mut FlowBoxChildBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("margin_start", val));
        value.connect_update(move |value| obj.set_property("margin_start", value));
        self.builder
    }
    pub fn vexpand_set(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut FlowBoxChildBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("vexpand_set", val));
        value.connect_update(move |value| obj.set_property("vexpand_set", value));
        self.builder
    }
    pub fn visible(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut FlowBoxChildBuilder {
        let obj = self.builder.obj.clone();
    ...
    pub fn css_name(&mut self, value: &(impl Prop<str> + ?Sized)) -> &mut FlowBoxChildBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("css_name", val));
        value.connect_update(move |value| obj.set_property("css_name", value));
        self.builder
    }
    pub fn valign(&mut self, value: &(impl Prop<Align> + ?Sized)) -> &mut FlowBoxChildBuilder {
        let obj = self.builder.obj.clone();
        value.with(|val| obj.set_property("valign", val));
        value.connect_update(move |value| obj.set_property("valign", value));
        self.builder
    }
    pub fn sensitive(&mut self, value: &(impl Prop<bool> + ?Sized)) -> &mut FlowBoxChildBuilder {
        let obj = self.builder.obj.clone();
...
pub struct FlowBoxChildSignals<'builder> {
    builder: &'builder mut FlowBoxChildBuilder,
}
impl<'builder> FlowBoxChildSignals<'builder> {
    pub fn child_notify(
        &mut self,
        f: impl Fn(&FlowBoxChild) + 'static,
    ) -> &mut FlowBoxChildBuilder {
        let obj = self.builder.obj.clone();
        obj.connect_child_notify(f);
        &mut self.builder
    }
    pub fn activate(&mut self, f: impl Fn(&FlowBoxChild) + 'static) -> &mut FlowBoxChildBuilder {
        let obj = self.builder.obj.clone();
        obj.connect_activate(f);
        &mut self.builder
    }
}
impl ForteExt for FlowBoxChild {
    type Builder = FlowBoxChildBuilder;
}
#[macro_export]
macro_rules ! FlowBoxChild { { $ ($ rest : tt) * } => { forte ! { (gtkrs :: FlowBoxChild :: forte ()) $ ($ rest) * } } }
